...
void Insert_outside()
{
...

	{
		...
		{
			MessageBox(NULL, TEXT("휴가 날짜입니다."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
			return 0;
		}
		...
		if (day > 31)
		{
			MessageBox(NULL, TEXT("31일 이내로 입력하시기 바랍니다."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
			return 0;
		}
		...
		if (start > 24)
		{
			MessageBox(NULL, TEXT("24시 이내로 입력하시기 바랍니다."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
			return 0;
		}
		...
		if (last > 24 || last<start)
		{
			MessageBox(NULL, TEXT("옳바른 시간을 입력해주세요."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
			return 0;
		}
...
void Insert_day()
{
	...
	if (retData->Schedule[day].day[0] == 1 || retData->Schedule[day].day[1] == 1)
	{
		MessageBox(NULL, TEXT("이미 휴가(반가)가 등록이 되어 있습니다."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
		return 0;
	}
   ...
   ///->외근 일정 기록은 남아있지만 휴가로 덮어씌워짐
	...
	for (int i = 0; i < 25; i++)
	{
		...
		{
			printf("%d %d\n", i, retData->Schedule[day].outside_duty[i]);
			...
			for (int j = 0; j < 25; j++)
			{
				printf("%d %d\n", j, retData->Schedule[day].outside_duty[j]);

				retData->Schedule[day].outside_duty[j] = 0;
...
void Insert_moning_half()
{
	...
	if (retData->Schedule[day].day[0] == 1 || retData->Schedule[day].day[1] == 1) {
		...
		MessageBox(NULL, TEXT("이미 휴가(반가)가 등록이 되어 있습니다."), TEXT("ERR"), MB_OK | MB_ICONSTOP);
...

{
	...
	if (stack[0][0])
	{
		for (int i = 0; i < top; i++)
		{
			...
			gotoxy(pos_start.X - strlen("%d번 : %d시 ~ %d시") / 2, pos_start.Y - 5 +j);
		...
		gotoxy(pos_start.X - strlen("어떤 외근을 삭제 하시겠습니까?") / 2, pos_start.Y + 7);

		printf("어떤 외근을 삭제 하시겠습니까? : ");

		gotoxy(pos_start.X + strlen("어떤 외근을 삭제 하시겠습니까?") / 2 + 3, pos_start.Y + 7);
		...
		MessageBox(NULL, TEXT("외근이 존재하지 않습니다."), TEXT("Check"), MB_OK | MB_ICONINFORMATION);
}
...

{
	...
	int top = 0, top1 = -1, day, j = 0, tmp1 = 0;
	float tmp = 3;
	...
	gotoxy(pos_start.X - strlen("휴가 : ") / 2 - 5, pos_start.Y - 1);
...

		{
...

			{
...

				{
					...
					gotoxy(pos_start.X + strlen("휴가 :   ") / 2 + j - 3 , pos_start.Y - 1);
				...
				else
				{
					...
					if (tmp != retData->count)
					{
						gotoxy(pos_start.X + strlen("휴가 : ") / 2 + j-4, pos_start.Y - 1);
			...
			else if (retData->Schedule[i - 1].day[0] != 1&&retData->Schedule[i - 1].day[1] != 1 &&retData->Schedule[i + 1].day[0] != 1 && retData->Schedule[i + 1].day[1] != 1)				//하루만 휴가를 사용한 경우
	...
	hImage = (HBITMAP)LoadImage(NULL, TEXT("일정.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	print_menuimage(hImage);
	gotoxy(pos_start.X - strlen("반차 : ") / 2-6, pos_start.Y - 1);
	j = 0;
	printf("반차 : ");
	...
	for (int i = 0; i < 32; i++)
	{
		...
		if (retData->Schedule[i].day[0] != 1 && retData->Schedule[i].day[1] == 1)
		{
			...
			gotoxy(pos_start.X + strlen("반차 :   ") / 2 + j - 3, pos_start.Y - 1);
			printf("%d ", i);		
			j += 3;
	...
	print_menuimage(hImage);
	int y = 0;
	gotoxy(pos_start.X - strlen("외근 :  ") / 2 -6, pos_start.Y - 1 + y);
	printf("외근 : ");
	...
	for (int day = 0; day < 32; day++)
	{
		...
		if (stackDay[day] != 0)			//외근 있는 날에 실행
		{
			...
			for (int j = 0; j < top; j++)
			{
				...
				gotoxy(pos_start.X - strlen("외근 :  10일 ->        ") / 2 +15, pos_start.Y - 1 + y);
